...
extension UserApiExtension on User {
  static User fromApiJson(Map<String, dynamic> json) {
    ...
    if (json['profile'] != null) {
      ...
      if (profileJson['serviceProviderProfile'] != null) {
        final spJson = profileJson['serviceProviderProfile'] as Map<String, dynamic>;
        serviceProviderProfile = ServiceProviderProfile(
          averageRating: (spJson['averageRating'] as num?)?.toDouble() ?? 0.0,
          yearsOfExperience: spJson['yearsOfExperience'] ?? 0,
          hourlyRate: (spJson['hourlyRate'] as num?)?.toDouble() ?? 0.0,
          categories: List<String>.from(spJson['categories'] ?? []),
          isAvailable: spJson['isAvailable'] ?? true,
          bio: spJson['bio'] ?? '',
          skills: List<String>.from(spJson['skills'] ?? []),
        );
      }
    ...
    return User(
      ...
      email: json['email'],
      userType: json['userType'] == 'client' ? UserType.client : UserType.serviceProvider,
...
extension ProjectApiExtension on Project {
  static Project fromApiJson(Map<String, dynamic> json) {
    ...
    return Project(
      ...
      priority: _parseProjectPriority(json['priority']),
      status: _parseProjectStatus(json['status']),
      clientId: json['client_id'],
      assignedProviderId: json['assigned_provider_id'],
      location: location,
  ...
  static ProjectPriority _parseProjectPriority(String priority) {
    switch (priority) {
      case 'low':
...
extension ServiceCategoryApiExtension on ServiceCategory {
  static ServiceCategory fromApiJson(Map<String, dynamic> json) {
